...
class payInfoVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        for item in RGPD.shared.theme.pages {
            if item.pageName == RGPDPages.payment_data.rawValue {
                self.theme = item
            }
        }
        descriptionLbl.text = RGPD.shared.texts.payment_data
    ...
    func setView() {
        ...
        if theme.emptyButtonColor.count == 1 {
            nextBtn.backgroundColor = colorGenerator.oneColor(theme.emptyButtonColor[0])
        } else {
    ...
    @IBAction func ContinueTapped(_ sender: Any) {
        if RGPD.shared.pagesShown.first == .payment_data {
    ...
    @IBAction func backPressed(_ sender: Any) {
        if RGPD.shared.pagesAccepted.contains(.payment_data) {
            RGPD.shared.pagesShown.insert(RGPD.shared.pagesAccepted.remove(at: RGPD.shared.pagesAccepted.firstIndex(of: .payment_data)!), at: 0)
        }
