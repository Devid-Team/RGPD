...
class commVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        for item in RGPD.shared.theme.pages {
            if item.pageName == RGPDPages.comm.rawValue {
                self.theme = item
            }
        }
        descriptionLbl.text = RGPD.shared.texts.comm
    ...
    @IBAction func ContinueTapped(_ sender: Any) {
        if RGPD.shared.pagesShown.first == .comm {
    ...
    @IBAction func backPressed(_ sender: Any) {
        if RGPD.shared.pagesAccepted.contains(.comm) {
            RGPD.shared.pagesShown.insert(RGPD.shared.pagesAccepted.remove(at: RGPD.shared.pagesAccepted.firstIndex(of: .comm)!), at: 0)
        }
